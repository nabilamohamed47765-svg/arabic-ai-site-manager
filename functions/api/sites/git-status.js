...
export async function onRequestPost(context) {
  try {
    ...
    await context.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(requestId, siteId, user.sub, "pending", "git_status", targetPath, now.toISOString(), expires.toISOString()).run();

    return Response.json({
      success: true, request_id: requestId, status: "pending",
      message: "تم إنشاء طلب فحص Git بنجاح"
    });

  } catch (error) {
    return Response.json({
      success: false, error: "حدث خطأ أثناء إنشاء طلب فحص Git", details: error.message
    }, { status: 500 });
  }
}