...
export async function onRequestPost(context) {
  try {
    ...
    if (!request) {
      return Response.json({ success: false, error: "طلب الكتابة غير موجود" }, { status: 404 });
    }
    ...
    const workflowFile = "ssh-write.yml";
    ...
    return Response.json({
      success: true, request_id: requestId, status: "running", message: "تم تشغيل كتابة الملف بنجاح"
    });

  } catch (error) {
    return Response.json({
      success: false, error: "حدث خطأ أثناء تشغيل كتابة الملف", details: error?.message || String(error)
    }, { status: 500 });
  }
}
