...
export async function onRequestPost(context) {
  try {
    ...
    if (!message) {
      message = "تحديث تلقائي عبر Arabic AI Site Manager";
    }
    ...
    await context.env.DB.prepare(`
      INSERT INTO ssh_requests (id, site_id, user_id, status, operation, target_path, file_content, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(requestId, siteId, user.sub, "pending", "git_commit", targetPath, message, now.toISOString(), expires.toISOString()).run();

    return Response.json({
      success: true, request_id: requestId, status: "pending",
      message: "تم إنشاء طلب Git Commit بنجاح"
    });

  } catch (error) {
    return Response.json({
      success: false, error: "حدث خطأ أثناء إنشاء طلب Git Commit", details: error.message
    }, { status: 500 });
  }
}
