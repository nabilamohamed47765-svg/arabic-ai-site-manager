...
export async function onRequestPost(context) {
  try {
    ...
    if (!request) {
      return Response.json({ success: false, error: "طلب Git Commit غير موجود" }, { status: 404 });
    }

    if (request.status !== "pending") {
      return Response.json({
        success: false, error: "طلب Git Commit ليس في حالة انتظار", request_status: request.status
      }, { status: 409 });
    }
    ...
    if (request.expires_at) {
      ...
      if (Number.isFinite(expiresAt) && expiresAt < Date.now()) {
        ...
        return Response.json({ success: false, error: "انتهت صلاحية طلب Git Commit" }, { status: 410 });
      }
    }
    ...
    const workflowFile = "ssh-git-commit.yml";
    ...
    if (!githubResponse.ok) {
      ...
      return Response.json({
        success: false, error: "فشل تشغيل GitHub Action",
    ...
    return Response.json({
      success: true, request_id: requestId, status: "running", message: "تم تشغيل Git Commit بنجاح"
    });

  } catch (error) {
    return Response.json({
      success: false, error: "حدث خطأ أثناء تشغيل Git Commit", details: error?.message || String(error)
    }, { status: 500 });
  }
}
