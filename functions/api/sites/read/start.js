...
export async function onRequestPost(context) {
  try {
    ...
    if (!request) {
      return Response.json({ success: false, error: "طلب القراءة غير موجود" }, { status: 404 });
    }

    if (request.status !== "pending") {
      return Response.json({
        success: false, error: "طلب القراءة ليس في حالة انتظار", request_status: request.status
      }, { status: 409 });
    }
    ...
    if (request.expires_at) {
      ...
      if (Number.isFinite(expiresAt) && expiresAt < Date.now()) {
        ...
        return Response.json({ success: false, error: "انتهت صلاحية طلب القراءة" }, { status: 410 });
      }
    }
    ...
    const workflowFile = "ssh-read.yml";
    ...
    return Response.json({
      success: true, request_id: requestId, status: "running", message: "تم تشغيل قراءة الملفات بنجاح"
    });

  } catch (error) {
    return Response.json({
      success: false, error: "حدث خطأ أثناء تشغيل قراءة الملفات", details: error?.message || String(error)
    }, { status: 500 });
  }
}
